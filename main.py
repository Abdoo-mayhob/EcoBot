...
# --------------------------------
# Created    : 2021 - 2 - 11
# LastEdited : 2021 - 6 - 22 12:25

# 2021.6.20 22:48 : If admin is abdoo dont log
# 2021.7.22 12:25 : Error Handling for Reading users_ids.txt, Edited /help & /start texts, BUG_FIX comments are not saved in tree
...
UNI_BOT_TOKEN = "ur test before launch bot token here"
ECO_BOT_TOKEN = "ur bot token here
TOKEN = ECO_BOT_TOKEN

UNIBOT_CLOUD_CHANNEL_ID = "-1001408055006"
UNI_BOT_ID = "1679876236"
ABDOO_ID = 507130741

LOG_GROUP_CHAT_ID = "-1001402842281"
ADMINS_IDS = {
    ABDOO_ID,
    None
...
+963 945 536 907
"""
SUPPORT_AND_ADMINS_HTML ="""
<u><b><em>الداعمين و الأدمن : </em></b></u>
الاتحاد الوطني لطلبة سورية - مكتب المعلوماتية الفرعي
الهيئة الإدارية في كلية الاقتصاد
"""
START_COMMAND_TEXT = """
تحياتي زميلتي  ❤
تحياتي زميلي  ❤
...
البوت لسا عم يتطور أي تعليق أو مشكلة
أو إقتراحات أنا جاهز دائماً،
...
<b>@Abdoo_M</b>
"""
OLD_START_COMMAND_TEXT ="""
تحياتي لكم ..
هاد البوت خاص بطلاب كلية الاقتصاد في جامعة البعث
موجود عليه الملفات المهمة يلي بيحتاجها الطالب خلال حياته الجامعية, من مقررات وأسئلة دورات وسلالم تصحيح .

<em>
الاتحاد الوطني لطلبة سورية - مكتب المعلوماتية
الهيئة الإدارية في كلية الاقتصاد
</em>
<b>@Abdoo_M </b>
"""
...
<b>/rename</b> \\n wht2rename(= . to rename current folder ) \n new_name
<b>/backup</b> \\n copyname
<b>/publication</b>
"""
# ===========================================================================================================================
...
def log_action(update, action: str):
    if update.message.from_user.id == ABDOO_ID:
        return
...
def load_users_ids():
    try:
        ...
        updater.bot.sendMessage(
        LOG_GROUP_CHAT_ID, f"Error While Loading Useres IDs:\n{E}", disable_notification=True)
        updater.bot.sendMessage(
        LOG_GROUP_CHAT_ID,f"Shutting Down The Bot ..:\n{E}", disable_notification=True)
        print("Error: ",E)
        print("-- EcoBot IS OFF --")
...
def help_command(update, context):
    ...
    #update.message.reply_text("+963 945 536 907")
    #update.message.reply_text(
    #    "facebook.com/Abdoo.Almayhob/", disable_web_page_preview=True)
    update.message.reply_text(SUPPORT_AND_ADMINS_HTML, parse_mode="HTML")
    ...
    try:
        ...
        if (inp in context.user_data["current_folder"].sub):
            ...
            if type(context.user_data["current_folder"].sub[inp]) == folder:
                ...
                notc = chosen_file.properties["num_of_times_called"]
                update.message.reply_text(f"هاد الملف صار مطلوب {notc} مرة 🤓")
...
def publication_command(update, context):
    ...
    try:
        ...
        log_action(update, f"Error While Publicating: \n{E}")
